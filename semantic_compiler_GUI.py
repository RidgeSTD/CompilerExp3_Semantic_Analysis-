...
#     print(result_listBox.size())
    ...
    result = semantic_parser_kernel.main(tmp_str)
    for x in xrange(0,len(result)):
        result_listBox.insert(END,str(x)+' \t '+result[x])
...
def onclick_file():
    ...
    result_listBox.delete(0, result_listBox.size())
    result_from_file = semantic_parser_kernel.main('')
    for x in xrange(0,len(result_from_file)):
        result_listBox.insert(END,str(x)+' \t '+result_from_file[x])
